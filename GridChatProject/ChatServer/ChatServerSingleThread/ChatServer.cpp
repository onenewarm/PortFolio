...
LONG s_SnapRecvTPS, s_SnapSentTPS, s_SnapUpdateTPS, s_SnapMsgReqTPS, s_SnapMsgResTPS;
...
void ChatServer::OnRecv(__int64 sessionID, CPacketRef& pkt)
{
	...
	if (job->type != EN_PACKET_CS_CHAT_REQ_LOGIN) {
		LONG jobQSize = m_UpdateJobQ->Enqueue(job);
		if (jobQSize == -1) {
		...
		LONG jobQSize = m_TokenJobQ->Enqueue(job);
		if (jobQSize == -1){
			OnSystemError(-1, L"TokenJobQ is fulled.", __FILE__, __LINE__);
			return;
		}
		if (jobQSize == 1)
...
void ChatServer::OnSessionError(__int64 sessionId, int errCode, const wchar_t* errMsg, const char* filePath, unsigned int fileLine)
{
	...
	{
		CPacketRef pkt = Make_EN_PACKET_CS_CHAT_RES_LOGIN_Packet(this, 1, 0);
...
void ChatServer::OnWorker(__int64 completionKey)
{
	...
	while (true) {
		if (m_MonitorClient->m_SessionId != NULL) {
			...
			m_MonitorClient->Request_EN_PACKET_SS_MONITOR_DATA_UPDATE_Packet(dfMONITOR_DATA_TYPE_CHAT_PLAYER, m_PlayerPool->m_UseNodeCount, timestamp);
...
bool ChatServer::Handle_EN_PACKET_CS_CHAT_REQ_LOGIN(__int64 sessionId, __int64 AccountNo, char* ID, char* Nickname, char* SessionKey)
{
	/*------------------------
	// Redis서버로부터 토큰 확인
	--------------------------*/
	if (CheckToken(AccountNo) == false)
	{
	...
	wmemcpy(player->m_Id, (wchar_t*)ID, 20);
	wmemcpy(player->m_NickName, (wchar_t*)Nickname, 20);
	SetLogin(sessionId);

	Job* job = m_JobPool->Alloc();
	job->type = Job::JOB_TYPE::PLAYER_CREATE;
	job->player = player;
	int jobQSize = m_UpdateJobQ->Enqueue(job);
	if (jobQSize == -1)
	{
		OnSystemError(0, L"UpdateJopQ was fulled.", __FILE__, __LINE__);
		return false;
	}

	if (jobQSize == 1) ::SetEvent(m_UpdateEvent);
	return true;
}
...
void ChatServer::Handle_PlayerCreate(Player* player)
{
	m_PlayerMap->insert({ player->m_SessionId, player });
	CPacketRef resPkt = Make_EN_PACKET_CS_CHAT_RES_LOGIN_Packet(this, dfCHAT_LOGIN_OK, player->m_AccountNo);
...
unsigned __stdcall ChatServer::Monitor(void* argv)
{
	...
	while (server->m_IsStop == false) {
		...
		StringCchPrintf(mLog, 4096, L"%sUpdateMsgQ : %d / PlayerPool : %d / PlayerCount : %d / PlayerMapSize : %d\n", mLog, server->m_UpdateJobQ->GetSize(), server->m_PlayerPool->GetChunkSize(), (LONG)server->m_PlayerMap->size(), (int)server->m_PlayerMap->bucket_count());
...
unsigned __stdcall ChatServer::Token(void* argv)
{
	ChatServer* server = (ChatServer*)argv;
	DWORD waitRet, errCode;
	...
	while (server->m_IsStop == false)
	{
		if (server->m_TokenJobQ->GetSize() == 0) {
			waitRet = WaitForSingleObject(server->m_TokenEvent, INFINITE);
			if (waitRet != WAIT_OBJECT_0) {
				errCode = GetLastError();
				server->OnSystemError(errCode, L"Failed wait token event.", __FILE__, __LINE__);
				continue;
			}
		}

		int remainJobSize = server->m_TokenJobQ->GetSize();

		while (remainJobSize--) {
			Job* job;
			server->m_TokenJobQ->Dequeue(&job);
			server->PacketProc(server, job->sessionId, job->pktRef);
			server->m_JobPool->Free(job);
		}
	}
	return 0;
...
ChatServer::ChatServer() : m_UpdateEvent(CreateEvent(NULL, false, false, NULL)), m_TokenEvent(CreateEvent(NULL, false, false, NULL)), m_UpdateThread(INVALID_HANDLE_VALUE), m_MonitorThread(INVALID_HANDLE_VALUE), m_CheckTokenThread(INVALID_HANDLE_VALUE), m_MaxPlayer(0), m_PlayerPool(new TLSObjectPool<Player>(0, false)), m_PlayerMap(new unordered_map<__int64, Player*>()), m_JobPool(new TLSObjectPool<Job>(0, true)), m_UpdateJobQ(new LockFreeQueue<Job*>(MAX_JOBQ_SIZE)), m_TokenJobQ(new LockFreeQueue<Job*>(MAX_JOBQ_SIZE)), m_RedisClient(NULL),  m_RecvMsgTPS(0), m_SendMsgTPS(0), m_UpdateTPS(0), m_MonitorClient((MonitorClient*)_aligned_malloc(sizeof(MonitorClient), 64))
{
	DWORD errCode;
	...
	m_CheckTokenThread = (HANDLE)::_beginthreadex(NULL, 0, Token, this, 0, &threadId);
...
void ChatServer::JobProc(Job* job)
{
	...
	case Job::JOB_TYPE::PLAYER_CREATE:
	{
		Handle_PlayerCreate(job->player);
		break;
	}
	default:
	{
...
bool ChatServer::CheckToken(LONG64 accountNo)
{
