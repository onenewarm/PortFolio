...
#define STATUS_ALLOC 0xBC
	...
	template<typename DATA, int CHUNK_SIZE>
	class TLSObjectPool
	{
		static constexpr DWORD MAX_CHUNK_SIZE = CHUNK_SIZE;
...
template<typename DATA, int CHUNK_SIZE>
inline DATA* onenewarm::TLSObjectPool<DATA, CHUNK_SIZE>::Alloc()
{
	Chunk* _chunk = (Chunk*)TlsGetValue(m_TlsIndex);
	if (_chunk == NULL) {
...
template<typename DATA, int CHUNK_SIZE>
inline bool onenewarm::TLSObjectPool<DATA, CHUNK_SIZE>::Free(DATA* addr)
{
...
template<typename DATA, int CHUNK_SIZE>
inline onenewarm::TLSObjectPool<DATA, CHUNK_SIZE>::TLSObjectPool(DWORD InitChunkCnt, bool PlacementNew) : m_UseNodeCount(0), m_ChunkPool(LockFreeObjectPool<typename TLSObjectPool<DATA, CHUNK_SIZE>::Chunk>(InitChunkCnt, PlacementNew))
{
	m_TlsIndex = ::TlsAlloc();
...
template<typename DATA, int CHUNK_SIZE>
inline onenewarm::TLSObjectPool<DATA, CHUNK_SIZE>::~TLSObjectPool()
{
	::TlsFree(m_TlsIndex);
}